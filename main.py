...
from crew_definition import DebugCrew
...
async def execute_crew_task(input_data: dict) -> str:
    """ Execute debug task with optional failure injection """
    logger.info(f"Starting debug task with input: {input_data}")
    ...
    crew = DebugCrew(logger=logger)
    
    # Extract text or use command as text
...
async def start_job(data: StartJobRequest):
    ...
    try:
        # Check for failure injection at start_job
        failure_type = failure_injector.should_fail(
...
async def handle_payment_status(job_id: str, payment_id: str) -> None: 
    """ Executes debug task after payment confirmation """
    try:
        ...
        logger.info(f"Debug task completed for job {job_id}")
...
def main():
    ...
    print("\n" + "=" * 70)
    print("🚀 Running Debug Agent locally (standalone mode)...")
    print("=" * 70 + "\n")
    
    # Define test input
    input_data = {"text": "Test debug agent functionality"}
    
    print(f"Input: {input_data['text']}")
    print("\nProcessing with Debug Agent...\n")
    
    # Initialize and run the crew
    crew = DebugCrew(verbose=True)
